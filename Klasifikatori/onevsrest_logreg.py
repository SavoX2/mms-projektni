...
if not cf_file.exists():
    ...
    for C in C_range:
        for solver in solvers:
            ...
            cf = OneVsRestClassifier(LogisticRegression(C=C,solver=solver, max_iter=1000))
            ...
            if acc > max_acc:
                print('Nova najbolja tacnost (', acc, '>',
                        max_acc, ') dala je kombinacija C = ', C, ', solver = ', solver)
                max_acc = acc
                best_C = C
                best_solver = solver
                acc_tuple = (acc, hamm, f1, C, time, inner_time_passed)
            if hamm < min_hamm:
                print('Novi najbolji hemming score (', hamm, '<',
                        min_hamm, ') dala je kombinacija C = ', C, ', solver = ', solver)
                min_hamm = hamm
                hamm_tuple = (acc, hamm, f1, C, time, inner_time_passed)
            if f1 > max_f1:
                print('Novi najbolji F1 score (', f1, '>',
                        max_f1, ') dala je kombinacija C = ', C, ', solver = ', solver)
                max_f1 = f1
                f1_tuple = (acc, hamm, f1, C, time, inner_time_passed)
            if inner_time_passed < time_passed:
                print('Novo najbrze vrijeme (', inner_time_passed, '<',
                        time_passed, ') dala je kombinacija C = ', C, ', solver = ', solver)
                time_passed = inner_time_passed
                time_tuple = (acc, hamm, f1, C, time, inner_time_passed)
    ...
    cf = OneVsRestClassifier(LogisticRegression(C=best_C, solver=best_solver, max_iter=1000))
