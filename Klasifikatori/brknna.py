    ...
    for k in np.arange(1, 6):
        ...
        if acc > max_acc:
            print('Nova najbolja tacnost (', acc, '>',
                    max_acc, ') dala je kombinacija k = ', k)
            max_acc = acc
            best_k = k
            acc_tuple = (acc, hamm, f1, k, time, inner_time_passed)
        if hamm < min_hamm:
            print('Novi najbolji hemming score (', hamm, '<',
                    min_hamm, ') dala je kombinacija k = ', k)
            min_hamm = hamm
            hamm_tuple = (acc, hamm, f1, k, time, inner_time_passed)
        if f1 > max_f1:
            print('Novi najbolji F1 score (', f1, '>',
                    max_f1, ') dala je kombinacija k = ', k)
            max_f1 = f1
            f1_tuple = (acc, hamm, f1, k, time, inner_time_passed)
        if inner_time_passed < time_passed:
            print('Novo najbrze vrijeme (', inner_time_passed, '<',
                    time_passed, ') dala je kombinacija k = ', k)
            time_passed = inner_time_passed
            time_tuple = (acc, hamm, f1, k, time, inner_time_passed)
