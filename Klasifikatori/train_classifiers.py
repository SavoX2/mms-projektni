...
def word2vec(x_data, pos_filter):
    ...
    google_vecs = api.load("word2vec-google-news-300").wv
...
def doc2vec(data_df):
    ...
    model = Doc2Vec(min_count=1, window=10, size=300,
                    sample=1e-4, negative=5, workers=2)
...
def main():
    ...
    data_df = pd.read_csv(
        "Klasifikatori/movies_genres_en.csv", delimiter='\t', nrows=10000, index_col=0)
    # all the list of genres to be used by the classification report
    genres = list(data_df.drop(
        ['title', 'plot', 'plot_lang'], axis=1).columns.values)
    ...
    if args.vectors == 'tfidf':
        ...
        data_y = data_df.drop(
            ['title', 'plot', 'plot_lang'], axis=1).as_matrix()
        print(data_x.shape)
        print(data_y.shape)
        stratified_split = ShuffleSplit(
            n_splits=2, test_size=0.33, random_state=42)
        for train_index, test_index in stratified_split.split(data_x, data_y):
            print(data_x[train_index])
            x_train, x_test = data_x[train_index], data_x[test_index]
            y_train, y_test = data_y[train_index], data_y[test_index]

        # transform matrix of plots into lists to pass to a TfidfVectorizer
        train_x = np.array([x[0].strip() for x in x_train.tolist()])
        test_x = np.array([x[0].strip() for x in x_test.tolist()])
        ...
        if args.classifier == 'nb':
            ...
            grid_search(train_x, y_train, test_x, y_test,
                        np.array(genres), parameters, pipeline)
            exit(-1)
    ...
    if args.vectors == 'word2vec':
        ...
        # need to transform back into numpy array to apply ShuffleSplit
